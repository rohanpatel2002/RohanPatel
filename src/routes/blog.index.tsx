  ...
  head: () => ({
    meta: [
      { title: "Blog — Rohan Patel" },
      {
        name: "description",
        content:
          "Essays on systems, AI, and software engineering by Rohan Patel.",
      },
    ],
  }),
  component: BlogIndex,
    ...
    <div className="grain min-h-screen bg-background">
      ...
      <section className="px-4 pt-20 sm:px-6 sm:pt-28 md:pt-32">
        <div className="mx-auto max-w-7xl text-center">
          ...
          <WordReveal
            text="Notes & essays."
            className="mt-4 font-display text-6xl sm:text-8xl md:text-[9rem]"
          />
          <Reveal delay={0.3}>
            <p className="mx-auto mt-6 max-w-xl text-base text-muted-foreground sm:mt-8 sm:text-lg">
              Notes on systems, AI, and shipping software. Longer essays ship
              when they are ready — theses live here first.
            </p>
          </Reveal>
        </div>
            ...
            <Reveal key={p.slug} delay={i * 0.08}>
              ...
              >
                <Link
                  to="/blog/$slug"
                  params={{ slug: p.slug }}
                  className="group flex flex-col gap-3 py-6 sm:py-8 md:flex-row md:items-baseline md:justify-between"
                >
                  <div className="flex-1">
                    ...
                    <h2 className="mt-2 font-display text-2xl transition-colors group-hover:text-accent sm:text-3xl md:text-5xl">
                      {p.title}
                    </h2>
